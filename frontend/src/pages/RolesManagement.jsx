...
const RolesManagement = () => {
  ...
  const resetForm = () => {
    ...
    setNewRolePermissions({ create: false, view: false, patch: false });
  };
  ...
  const handleEditRole = (role) => {
    ...
    const permissions = {
      create: (role.Permissions || []).some(p => p.action.toLowerCase().includes('create')),
      view: (role.Permissions || []).some(p => p.action.toLowerCase().includes('view')),
      patch: (role.Permissions || []).some(p => p.action.toLowerCase().includes('update') || p.action.toLowerCase().includes('patch'))
    };
